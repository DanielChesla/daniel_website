...
- **TECH-010 (revised 2026-07-16):** The function must enforce, server-side (not just trust the client or the model):
  - The 20-question cap and forced-guess rule (FR-009a): once `history.length >= 20`, the function must return a guess. If the model does not comply (still returns `"questions"`), that is treated as an `invalid_response` error (TECH-007b) rather than fabricating a guess from question text.
  - The final-guess rule (FR-009): once `guessesSoFar.length >= 1`, the function must return a guess, with the same non-compliance handling as above.
