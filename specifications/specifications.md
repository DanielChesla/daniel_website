...
>
> **Revision (2026-07-17) — 5 batches of 4, for guessing accuracy:** The batching strategy changed from **4 batches of 5** to **5 batches of 4** (checkpoints at 0/4/8/12/16 answered questions), so each batch can be given a distinct, progressively narrowing purpose (broad category → major characteristics → shortlist → distinguish candidates → targeted confirmation) and the model is explicitly instructed to choose each question for information gain against its current shortlist. Early guessing is now disallowed before question 8, and only permitted from question 8 through 19 when the model declares "high" confidence (an internal-only signal, never shown in the UI — see FR-006d/FR-006e, OUT-007). The maximum number of AI calls per round, excluding client-side retries, is now **7** (5 batch calls + 1 forced first-guess call + 1 final-guess call), up from 6. See the revision notes in Sections 6, 11, and 13.
...
### Question Flow (revised 2026-07-16 — batched adaptive questioning hotfix; revised again 2026-07-17 — 5 batches of 4)
> **Revision note:** The original design called the serverless function once per question. Since anonymous/keyless upstream calls can be slow (TECH-008) and this is a low-traffic free-tier integration, the game was changed to request questions in batches, reducing the number of AI calls per round from up to 21 down to a small, bounded number. The batch size was subsequently changed from **4 batches of 5** to **5 batches of 4** to allow each batch to carry a distinct, progressively narrowing purpose and improve guessing accuracy (see FR-006d/FR-006e below). FR-003/FR-006/FR-009 reflect this revised architecture; the user-visible one-question-at-a-time flow, five answer choices, progress bar, and visual design are unchanged.

- **FR-003 (revised 2026-07-17):** The game is played in **batches of up to four yes/no questions per AI request**, not one AI request per question. At the start of a round, and again every time the client has exhausted the current locally-held batch (checkpoints at 4, 8, 12, and 16 answered questions in the common case), the client requests the next batch (or a guess) from `/api/twenty-questions`. The client then presents the returned questions to the user **one at a time**, purely client-side, with no additional network requests until that batch is exhausted. The round is capped at a maximum of 20 total answered questions (see FR-009a).
...
- **FR-006:** The AI decides dynamically (via the serverless function/LLM call) whether to return another batch of questions or make a guess, based on the accumulated question/answer history, subject to the early-guess rules in FR-006d/FR-006e below.
...
- **FR-006b (revised 2026-07-17):** Whenever the AI chooses to return questions rather than a guess, the response must contain **exactly `min(4, 20 - questionsAskedSoFar)` new, unique questions** (enforced server-side, see TECH-007/TECH-010). This guarantees that, in the worst case (the AI never guesses early), the round reaches 20 answered questions via **at most five** batch-generation calls (checkpoints at 0/4/8/12/16).
...
- **FR-006d (NEW, 2026-07-17 — staged batch purposes & information gain):** Each of the five 4-question batches must be given a distinct purpose in the prompt sent to the LLM, to progressively narrow the space of plausible answers:
  | Questions | Purpose |
  |---|---|
  | 1–4 | Determine the broad category (person, place, animal, or object) and the most fundamental attributes |
  | 5–8 | Narrow down major characteristics — geography, era, use, or domain |
  | 9–12 | Establish a shortlist of plausible specific answers |
  | 13–16 | Distinguish between the leading candidates on that shortlist |
  | 17–20 | Ask highly targeted confirmation questions |

  Additionally, the model must be instructed to choose each question for **information gain**: a question should meaningfully split the current set of plausible candidates into roughly balanced groups, rather than confirming something already strongly suspected.
- **FR-006e (NEW, 2026-07-17 — early-guess confidence gating):** The AI must not make any guess (early or otherwise) before **8 total questions have been answered**. From question 8 through question 19, the AI may make an early guess only when it declares **"high"** confidence via an internal-only `confidence` field (`"high"|"medium"|"low"`) on the guess response; a guess declared with `"medium"`/`"low"` confidence, or with the field omitted, is rejected by the server as an `invalid_response` error in exactly the same way as any other malformed model output (no special-cased handling, no internal retry — see TECH-007b/TECH-010), and the client's existing bounded retry flow (FR-016, 3 total attempts) handles it, so persistent non-compliance cannot loop forever — it terminates at the normal manual-Retry error state. This `confidence` field is **server-internal only and is never surfaced to the user** in any UI (consistent with OUT-007 — no numeric/qualitative confidence is displayed during gameplay). Confidence is not required (and is ignored if present) on the forced first guess at the 20-question cap or on the second/final guess (FR-009), since those calls must produce a guess unconditionally.
...
- **TECH-007 (revised 2026-07-17 — 5 batches of 4):** The function must construct a prompt instructing the LLM to act as a 20-Questions guesser, review the history, and respond with **strict, parseable JSON** in one of these shapes:
  - `{ "type": "questions", "questions": ["Is it man-made?", "Is it bigger than a car?", "..."] }` — an array of **exactly `min(4, 20 - questionsAsked)`** new, unique yes/no questions (replaces the original single-question `{"type":"question","text":...}` shape).
  - `{ "type": "guess", "text": "a golden retriever", "confidence": "high"|"medium"|"low" }` — `confidence` is required for early (non-forced) guesses and ignored on forced/final guesses (see FR-006e, TECH-007d).
  The request to Pollinations.ai must include **`"response_format": { "type": "json_object" }`** (verified supported by Pollinations.ai's `POST https://text.pollinations.ai/openai` endpoint) in addition to prompt-level instructions, to improve the reliability of getting clean, parseable JSON back. The prompt must also convey the current stage's purpose (FR-006d) and the information-gain instruction for that turn's batch request. The function must validate, deduplicate, and sanitize this JSON server-side (see TECH-010) before returning it to the client, and return a clear error if the LLM output can't be parsed or used, so the client's retry logic (FR-016) can engage.
...
- **TECH-007d (NEW, 2026-07-17 — early-guess confidence gating):** When the model returns a guess and the turn is not a forced/final-guess turn (i.e., `guessesSoFar` is empty and the 20-question cap has not been reached), the function must reject the guess as an `invalid_response` error (same shape/status as any other invalid model output, no internal retry per TECH-007b) when either:
  - fewer than 8 total questions have been answered so far (an early guess is never permitted this early, regardless of declared confidence), or
  - the declared `confidence` field is anything other than exactly `"high"` (including `"medium"`, `"low"`, or the field being missing/unparseable).
  Because this uses the same reject-and-let-the-client-retry mechanism as every other invariant in this function, persistent non-compliance (e.g., a model that keeps declaring `"medium"` confidence) cannot cause an infinite loop — it is bounded by the client's existing 3-total-attempt retry policy (FR-016) and terminates normally at the manual-Retry error state.
...
- **TECH-010 (revised 2026-07-17):** The function must enforce, server-side (not just trust the client or the model):
  ...
  - Exact batch sizing (FR-006b): a batch must contain exactly `min(4, 20 - questionsAsked)` new, unique questions after deduplication against the full history and within the batch itself; a short batch after filtering is an `invalid_response` error (no internal retry, per TECH-007b).
  - Early-guess gating (FR-006e/TECH-007d): no guess before question 8, and only a `"high"`-confidence guess accepted from question 8 through 19.
  ...
  Together these guarantee a round completes in **at most five batch-generation calls (checkpoints 0/4/8/12/16), plus one first-guess call, plus one final-guess call if the first is rejected (seven calls total)** — not counting the client's own automatic retries of a failed call (FR-016), which retry the same logical turn rather than adding a new one.
...
- **TEST-008 (revised 2026-07-17 — 5 batches of 4):** A test that stubs a single batch response of 4 questions, answers all 4 locally, and asserts only **one** API call was made until the 4th answer triggers the next request (verifying the batching/local-rendering behavior of FR-003/FR-006b).
- **TEST-009 (revised 2026-07-17 — 5 batches of 4):** A test that uses a stateful route stub (reading the posted `history`/`guessesSoFar` from each request, and never declaring early-guess confidence, to deterministically exercise the worst case) to drive a full round to the 20-question cap, asserting the forced first guess arrives after exactly 6 API calls (5 batches + 1 forced guess — TECH-010), and that rejecting it triggers exactly one further "final guess" call (7 total — FR-009).
...
- **TEST-012 (revised 2026-07-17 — `tests/twenty-questions-api.spec.js`):** Focused, offline tests that call the `api/twenty-questions.js` handler directly (mocking `global.fetch` to simulate Pollinations.ai, no browser/page fixture needed) covering: a forced guess correctly returned after 20 answered questions; an upstream response with a **missing `message.content`** field being rejected as `upstream_error`; the model returning `"questions"` when a guess is mandatory being rejected as `invalid_response`; a valid forced-guess response being accepted and passed through; an empty guess and a repeated/previously-rejected guess both being rejected; that the outgoing request body includes `reasoning_effort: "low"` alongside `response_format: { type: "json_object" }`; and (2026-07-17) that the batch target is exactly 4 at both the first checkpoint (empty history) and the final pre-cap checkpoint (16 answered).
- **TEST-013 (NEW, 2026-07-17 — early-guess confidence gating, `tests/twenty-questions-api.spec.js`):** Focused, offline tests covering: an early guess attempted before question 8 being rejected as `invalid_response` even when declared `"high"` confidence; an early guess at question 8 with `"high"` confidence being accepted; an early guess at question 8 with only `"medium"` confidence being rejected; an early guess at question 12 with a missing `confidence` field being rejected; and a forced guess at the 20-question cap with no `confidence` field at all still being accepted (confidence is irrelevant once forced).
- **TEST-014 (NEW, 2026-07-17 — bounded retries, no infinite loop):** Two tests proving persistent non-compliance cannot loop forever: (1) an offline test (`tests/twenty-questions-api.spec.js`) calling the handler 3 times in a row with a persistently low-confidence early guess, asserting every independent call returns the same well-formed `invalid_response` error rather than throwing/hanging; (2) an E2E test (`tests/labs.spec.js`) stubbing a persistent `invalid_response` (502) on every request, asserting the client makes exactly 3 total attempts (FR-016) before showing the generic error message and manual "Retry" button — never retrying indefinitely.
...
- **OUT-007:** Displaying a numeric or qualitative confidence score to the end user during gameplay (confidence-based reasoning is explained conceptually in the "About This Lab" copy only, not surfaced as a live UI metric). This includes the server-internal `confidence` field used for early-guess gating (FR-006e/TECH-007d) — it governs server-side accept/reject logic only and is never rendered in the client UI.
